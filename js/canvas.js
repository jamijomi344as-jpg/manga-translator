...
const CanvasManager = (() => {
  ...
  function loadImage(dataUrl, meta) {
    if (!_canvas) return;
    // Pass { crossOrigin: null } so blob: URLs are never blocked by CORS.
    fabric.Image.fromURL(dataUrl, (img) => {
      // Safety guard — Fabric can silently call back with null/broken img.
      if (!img || !img.width || !img.height) {
        if (typeof Toast !== 'undefined') Toast.show('Image loading failed — unsupported format or corrupt file.', 'error');
        console.error('[CanvasManager] loadImage: received null/empty Fabric image object.');
        return;
      }
  ...
  function replaceBackground(dataUrl) {
    ...
    fabric.Image.fromURL(dataUrl, (img) => {
      if (!img || !img.width || !img.height) {
        if (typeof Toast !== 'undefined') Toast.show('Cleaned image could not be displayed.', 'error');
        return;
      }
